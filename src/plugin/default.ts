...
import { ELASTICSEARCH_STATUS, ELASTICSEARCH_STATUS_LOG } from '../config/elasticsearch';
  ...
  model.esTruncate((error) => {
    ...
    stream.on('data', async (err, doc) => {
      ...
      try {
        ...
        if (ELASTICSEARCH_STATUS_LOG === 'enabled') logger.warn('mongoosastic-data-addFields %o : %o', title, count);
      } catch (error) {
